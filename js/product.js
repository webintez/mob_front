...
function setupEventListeners() {
    ...
    const deliveryPincodeInput = document.getElementById('deliveryPincode');
    if (deliveryPincodeInput) {
        let pincodeDebounceTimer;
        deliveryPincodeInput.addEventListener('input', (e) => {
            const val = e.target.value.trim();
            // Clear previous error on typing
            const errorContainer = document.getElementById('pincodeErrorMessage');
        ...
        deliveryPincodeInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                checkDelivery();
            }
        });
    ...
    if (mainImageContainer) {
        ...
        mainImageContainer.addEventListener('mouseleave', hideImageZoom);
    }
...
function displayProduct(product) {
    ...
    setTimeout(() => {
        ...
        if (window.innerWidth > 768) {
            ...
            // Mobile: Initialize tabs for details
            initMobileTabs();
        }
    }, 100);
}
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimeout = setTimeout(() => {
            ...
            if (window.innerWidth > 768) {
                ...
                // Mobile: Re-initialize tabs on resize
                initMobileTabs();
            }
        }, 100);
    });
...
// Helper to show/hide pincode error right underneath the input field
function showPincodeError(msg) {
    let errorContainer = document.getElementById('pincodeErrorMessage');
    if (!errorContainer) {
        errorContainer = document.createElement('div');
        errorContainer.id = 'pincodeErrorMessage';
        errorContainer.style.cssText = 'color: #ff6161; font-size: 12px; margin-top: 6px; text-align: left; padding-left: 2px;';
        const inputGroup = document.querySelector('.delivery-input-group');
        if (inputGroup) inputGroup.parentNode.insertBefore(errorContainer, inputGroup.nextSibling);
    }
    if (errorContainer) {
...
async function checkDelivery() {
    ...
    if (!pincode || !/^[1-9][0-9]{5}$/.test(pincode)) {
        showPincodeError('Please enter a valid 6-digit pincode (cannot start with 0)');
        return;
    }

    // Hide previous error explicitly
    const errorContainer = document.getElementById('pincodeErrorMessage');
    if (errorContainer) errorContainer.style.display = 'none';
    ...
    try {
        let apiUrl = `/api/delivery/check?pincode=${encodeURIComponent(pincode)}`;
        if (currentProduct && currentProduct.id) {
            apiUrl += `&product_id=${currentProduct.id}`;
        }

        // Use direct fetch so we can catch 422 validations without throwing generic errors
        const response = await fetch(apiUrl, {
            method: 'GET',
            headers: typeof API_CONFIG !== 'undefined' && API_CONFIG.headers ? API_CONFIG.headers : {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            }
        });
        ...
        if (!response.ok || !result.success) {
            // Validation error
            if (response.status === 422 && result?.errors?.pincode && result.errors.pincode[0]) {
                showPincodeError(result.errors.pincode[0]);
            } else {
                const errorMsg = result?.message || 'Failed to check delivery for this pincode';
                if (result?.errors) {
                    const errorList = Object.values(result.errors).flat().join(', ');
                    showPincodeError(errorList || errorMsg);
                } else {
                    showPincodeError(errorMsg);
                }
            }
        ...
        if (deliveryData.serviceable === false) {
            showPincodeError(deliveryData.message || 'Currently not deliverable to this PIN code');
            deliveryInfo.style.display = 'none';
            return;
        }
        ...
        if (deliveryDateDetail) {
            deliveryDateDetail.textContent = deliveryMessage || deliveryDate;
        }
        ...
        showPincodeError('Unable to check delivery for this pincode. Please try again.');
        deliveryInfo.style.display = 'none';
    } finally {
        if (checkBtn) {
...
// ============================================================
// Image Zoom: Lens-on-image + Result panel (Flipkart/Amazon style)
// ============================================================

function handleImageZoom(e) {
    const mainImage = document.getElementById('mainProductImage');
    const lens = document.getElementById('imageZoomLens');
    const resultPanel = document.getElementById('zoomResultPanel');

    if (!mainImage || !lens || !resultPanel || !mainImage.src || mainImage.src.endsWith('placeholder.jpg')) return;

    const imgRect = mainImage.getBoundingClientRect();
    const containerRect = mainImage.parentElement.getBoundingClientRect(); // main-image-wrapper

    // ZOOM MAGNIFICATION
    const ZOOM_FACTOR = 4;

    // Lens size (in px) — must match CSS
    const lensW = 130;
    const lensH = 130;
    ...
    // If cursor is outside the image bounds, hide
    if (cursorX < 0 || cursorY < 0 || cursorX > imgRect.width || cursorY > imgRect.height) {
        lens.style.display = 'none';
        resultPanel.style.display = 'none';
        return;
    }

    // Lens center should follow cursor — clamp so lens stays within image bounds
    let lensLeft = cursorX - lensW / 2;
    let lensTop = cursorY - lensH / 2;
    lensLeft = Math.max(0, Math.min(lensLeft, imgRect.width - lensW));
    lensTop = Math.max(0, Math.min(lensTop, imgRect.height - lensH));

    // The lens position is RELATIVE TO the main-image-wrapper (position:relative)
    const wrapperOffsetX = imgRect.left - containerRect.left;
    const wrapperOffsetY = imgRect.top - containerRect.top;
    lens.style.left = `${wrapperOffsetX + lensLeft}px`;
    lens.style.top = `${wrapperOffsetY + lensTop}px`;
    lens.style.display = 'block';

    // Result panel background: zoom into the image at the correct position
    // backgroundSize = image rendered size * ZOOM_FACTOR
    const bgW = imgRect.width * ZOOM_FACTOR;
    const bgH = imgRect.height * ZOOM_FACTOR;

    // The background position to show the area under the lens center
    const lensCenterXInImage = lensLeft + lensW / 2;
    const lensCenterYInImage = lensTop + lensH / 2;

    const resultPanelW = 500;
    const resultPanelH = 500;

    // Convert lens center to zoomed image coordinates
    const bgX = (lensCenterXInImage / imgRect.width) * bgW - resultPanelW / 2;
    const bgY = (lensCenterYInImage / imgRect.height) * bgH - resultPanelH / 2;

    resultPanel.style.backgroundImage = `url(${mainImage.src})`;
    resultPanel.style.backgroundSize = `${bgW}px ${bgH}px`;
    resultPanel.style.backgroundPosition = `-${bgX}px -${bgY}px`;
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            if (result.data.variation_attributes && result.data.variation_attributes.length > 0) {
                ...
                // Clear skeleton if no variations exist
                const variations = document.getElementById('variationsDetailsSection');
                if (variations) variations.innerHTML = '';
            }
        } else {
            // Clear skeleton if call fails or was unsuccessful
            const variations = document.getElementById('variationsDetailsSection');
            if (variations) variations.innerHTML = '';
        }
    } catch (error) {
...
function displayWarrantyInfo(warranty, brand) {
    ...
    // Show the warranty row
    const warrantyRow = document.querySelector('.warranty-row');
    if (warrantyRow) {
    ...
    if (deviceWarranty && accessoriesWarranty) {
        ...
        // Hide the section if no specific warranty text is available
        if (warrantyRow) {
...
async function loadAndUpdateProductForVariation(variationProduct) {
    ...
    if (variationProduct.slug) {
        ...
        const currentSlug = getProductSlugFromURL();
        if (currentSlug && currentSlug !== variationProduct.slug) {
            // Perform redirect to the correct variant page
            window.location.replace(`/product.html?slug=${encodeURIComponent(variationProduct.slug)}`);
            return;
        }
...
function displayGroupedSpecifications(specifications) {
    ...
    specifications.forEach((section, sectionIndex) => {
        ...
        specsTable.className = 'specifications-grid specifications-table';
        specsTable.id = `specificationsTable-${sectionIndex}`;

        // Add rows
        section.rows.forEach((row, rowIndex) => {
            const rowElement = document.createElement('div');
            rowElement.className = 'specification-item';
    ...
    // Apply collapse toggle for Specifications
    if (typeof applyCollapseToggle === 'function') {
        applyCollapseToggle(specsContainer, 'Specifications');
    }
}
...
function displaySpecificationsInDetails(attributes) {
    ...
    specsTable.className = 'specifications-grid';
...
function displaySimilarProducts(products) {
    ...
    if (products && products.length > 0) {
        ...
        if (carousel.children.length > 0) {
            ...
            setupSimilarProductsLoop(carousel);
        } else {
            section.style.display = 'none';
        }
    } else {
...
// Function for Infinite Loop on Similar Products (Manual Scroll)
function setupSimilarProductsLoop(carousel) {
    if (!carousel) return;

    const items = Array.from(carousel.children);
    if (items.length <= 1) return;

    // Record original width before cloning
    const originalScrollWidth = carousel.scrollWidth;

    // 1. Clone & Append (for scrolling right)
    items.forEach(item => {
        carousel.appendChild(item.cloneNode(true));
    });

    // 2. Clone & Prepend (for scrolling left)
    // Prepend in reverse order to maintain visual order
    for (let i = items.length - 1; i >= 0; i--) {
        carousel.insertBefore(items[i].cloneNode(true), carousel.firstChild);
    }

    // Layout is now: [Left Clones] [Originals] [Right Clones]
    // Width of Left Clones = originalScrollWidth
    // Start view at the beginning of the Originals
    setTimeout(() => {
        if (carousel) {
            carousel.scrollLeft = originalScrollWidth;
        }
    }, 0);

    let isAdjusting = false; // Flag to prevent event loop storms
    ...
    carousel.addEventListener('scroll', () => {
        ...
        // Scrolled too far right (into right clones)
        if (currentScroll >= originalScrollWidth * 2) {
            isAdjusting = true;
            carousel.scrollLeft = currentScroll - originalScrollWidth;
            isAdjusting = false;
        }
        // Scrolled too far left (into left clones)
...
function displayLongDescription(longDescription) {
    ...
    if (longDescription) {
        ...
        if (Array.isArray(descriptionData) && descriptionData.length > 0) {
            ...
            // Check if all items are image-only (image_url present, heading and short_text empty)
            const isImageOnly = descriptionData.every(item =>
                item.image_url &&
                (!item.heading || item.heading.toString().trim() === '') &&
                (!item.short_text || item.short_text.toString().trim() === '')
            );
            ...
            descriptionData.forEach((item, index) => {
                ...
                if (isImageOnly) {
                    // Image only layout: stacked and full width
                    featureBlock.style.display = 'block';
                    featureBlock.style.marginBottom = '20px'; // Closer spacing for contiguous images
                    featureBlock.style.width = '100%';

                    const img = document.createElement('img');
                    img.src = item.image_url;
                    img.alt = 'Product feature';
                    img.style.width = '100%';
                    ...
                    featureBlock.appendChild(img);
                } else {
                    // Original layout: flex row with text & image
        ...
        // Apply collapse toggle for Description
        if (typeof applyCollapseToggle === 'function') {
            applyCollapseToggle(productDescriptionContent, 'Description');
        }
    } else {
...
// Helper function to apply See More / See Less toggle
function applyCollapseToggle(container, sectionName) {
    if (!container) return;

    // Remove existing toggle to prevent duplicates on re-render
    const existingToggle = container.parentNode.querySelector('.see-more-toggle-container');
    ...
    setTimeout(() => {
        if (container.offsetHeight > maxHeight) {
            ...
            gradient.className = 'see-more-gradient';
            gradient.style.position = 'absolute';
            gradient.style.bottom = '0';
            gradient.style.left = '0';
            gradient.style.width = '100%';
            gradient.style.height = '80px';
            gradient.style.background = 'linear-gradient(to bottom, rgba(255,255,255,0), rgba(255,255,255,1))';
            gradient.style.pointerEvents = 'none';
            container.appendChild(gradient);

            // Create button container
            const toggleWrapper = document.createElement('div');
            toggleWrapper.className = 'see-more-toggle-container';
            toggleWrapper.style.textAlign = 'center';
            toggleWrapper.style.padding = '15px 0';
            toggleWrapper.style.background = 'white';
            toggleWrapper.style.borderTop = '1px solid #f0f0f0';

            const btn = document.createElement('button');
            btn.className = 'see-more-toggle-btn';
            btn.innerHTML = `See More <i class="fas fa-chevron-down" style="font-size: 12px; margin-left: 4px;"></i>`;
            btn.style.color = '#2874f0'; // standard Blue
            ...
            btn.addEventListener('click', () => {
                ...
                if (isCollapsed) {
                    ...
                    btn.innerHTML = `See Less <i class="fas fa-chevron-up" style="font-size: 12px; margin-left: 4px;"></i>`;
                } else {
                    container.style.maxHeight = maxHeight + 'px';
...
function showLoading() {
    ...
    if (loadingSpinner) {
        // Hide spinner overlay on mobile to allow skeleton view
        loadingSpinner.style.display = isMobile ? 'none' : 'flex';
    }
    if (productContent) {
        // Keep layout frame visible on mobile for skeletons
        productContent.style.display = isMobile ? 'grid' : 'none';
    }

    // Inject Skeleton markup for Mobile
    if (isMobile) {
        const mainImageContainer = document.querySelector('.product-images-wrapper .main-image-container');
        if (mainImageContainer) {
            mainImageContainer.innerHTML = '<div class="skeleton skeleton-image"></div>';
        }

        const title = document.getElementById('productTitle');
        if (title) {
            title.innerHTML = '<div class="skeleton skeleton-text title-1"></div><div class="skeleton skeleton-text title-2"></div>';
        }

        const rating = document.getElementById('productRating');
        if (rating) {
            rating.innerHTML = '<div class="skeleton skeleton-text rating"></div>';
        }

        const price = document.getElementById('currentPrice');
        if (price) {
            price.innerHTML = '<div class="skeleton skeleton-text price"></div>';
        }

        const variations = document.getElementById('variationsDetailsSection');
        if (variations) {
            variations.innerHTML = '<div class="skeleton skeleton-text options-1"></div><div class="skeleton skeleton-text options-2"></div>';
        }
    }
}
...
async function submitReview(form) {
    ...
    try {
        ...
        try {
            result = await response.json();
            console.log(`Review API Response [POST /reviews]:`, result);
        } catch (jsonError) {
...
/**
 * Mobile Tabbed View & Show More Functionality
 * Applies only below 768px screen width
 */
function initMobileTabs() {
    if (window.innerWidth > 768) {
        // If switched to desktop, restore sections and remove tabs
        const container = document.getElementById('mobileTabsContainer');
        if (container && container.classList.contains('mobile-tabs-section')) {
            container.innerHTML = '';
            container.className = 'mobile-tabs-placeholder';
            document.body.classList.remove('tabs-active');

            // Restore sections
            document.querySelectorAll('.moved-to-tabs').forEach(el => {
                el.style.display = '';
                el.classList.remove('moved-to-tabs');
            });
        }
        return;
    }
    ...
    // Don't re-init if already active on mobile
    if (container.classList.contains('mobile-tabs-section')) return;

    // Target sections
    const highlightsSection = document.querySelector('.product-details-grid');
    const descriptionSection = document.getElementById('productDescriptionSection');
    const specsSection = document.getElementById('specificationsDetailSection');

    if (!highlightsSection && !descriptionSection && !specsSection) return;

    // Clear placeholder
    container.innerHTML = '';
    container.className = 'mobile-tabs-section';

    // Create structure
    const header = document.createElement('div');
    header.className = 'mobile-tabs-header';
    header.innerHTML = '<h2 class="mobile-tabs-title">All details</h2>';

    // Add collapse toggle if header needs to be clickable
    const toggle = document.createElement('button');
    toggle.className = 'mobile-tabs-toggle';
    ...
    const tabs = [];
    if (highlightsSection) tabs.push({ id: 'tab-showcase', label: 'Highlights', element: highlightsSection });
    if (specsSection) {
        tabs.push({ id: 'tab-specs', label: 'Specifications', element: specsSection });
    }

    if (descriptionSection) {
        let descLabel = 'Showcase';
        if (currentProduct && currentProduct.long_description) {
            try {
                let descriptionData = currentProduct.long_description;
                if (typeof descriptionData === 'string' && (descriptionData.trim().startsWith('[') || descriptionData.trim().startsWith('{'))) {
                    descriptionData = JSON.parse(descriptionData);
                }

                if (Array.isArray(descriptionData) && descriptionData.length > 0 && descriptionData[0].heading) {
                    descLabel = descriptionData[0].heading;
                }
            } catch (e) {
                // If parsing fails or it's HTML, keep 'Showcase'
            }
        }
        tabs.push({ id: 'tab-desc', label: descLabel, element: descriptionSection });
    }

    const contentArea = document.createElement('div');
    contentArea.className = 'mobile-tabs-content';

    tabs.forEach((tab, index) => {
        const btn = document.createElement('button');
        btn.className = `mobile-tab-btn ${index === 0 ? 'active' : ''}`;
        btn.textContent = tab.label;
        btn.onclick = () => switchMobileTab(tab.id, btn);
        nav.appendChild(btn);
        ...
        innerContent.className = 'truncated-content';

        // Move the content
        while (tab.element.firstChild) {
            innerContent.appendChild(tab.element.firstChild);
        }
        ...
        showMoreBtnWrapper.className = 'show-more-btn-wrapper';
        const showMoreBtn = document.createElement('button');
        showMoreBtn.className = 'show-more-btn';
        showMoreBtn.innerHTML = 'See more <i class="fas fa-chevron-down"></i>';
        showMoreBtn.onclick = () => {
            innerContent.classList.toggle('expanded');
            showMoreBtn.classList.toggle('expanded');
            if (innerContent.classList.contains('expanded')) {
                showMoreBtn.innerHTML = 'See less <i class="fas fa-chevron-up"></i>';
            } else {
                showMoreBtn.innerHTML = 'See more <i class="fas fa-chevron-down"></i>';
        ...
        content.appendChild(showMoreBtnWrapper);

        contentArea.appendChild(content);

        // Hide original section
        tab.element.style.display = 'none';
        tab.element.classList.add('moved-to-tabs');
    });
...
function switchMobileTab(tabId, clickedBtn) {
    // Buttons
