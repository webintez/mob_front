...
function getPageSlugFromURL() {
    ...
    // Check for query parameter first (e.g., page.html?slug=about)
    ...
    // Handle direct slug (e.g., /about-us) if not just index.html or empty
...
async function loadPageContent() {
    ...
    // Add slug-based class to body for targeted styling
    document.body.classList.add('page-' + slug);

    // Use global AUTH_CONFIG if available, otherwise fallback (though auth.js should be loaded)
    ...
    try {
        ...
        // If page not found, try fetching as a section group
        try {
            const groupResponse = await fetch(`${baseUrl}/section-groups/${slug}`, {
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                    'X-API-Key': apiKey
            ...
            if (groupResponse.ok && groupData.success && groupData.data) {
                displaySectionGroup(groupData.data);
                return;
            }
        } catch (groupError) {
            console.error('Error loading section group:', groupError);
        }
...
function scopeStyles(css) {
    ...
    // BRAND COLOR REPLACEMENT: Global check for legacy colors in injected content
    // Replace Flipkart Blue (#2874f0) with brand green (#0a4409)
    scoped = scoped.replace(/#2874f0/gi, '#0a4409');
    // Replace light blue highlight (#e3f2fd) or similar with brand green highlight
    scoped = scoped.replace(/#e3f2fd/gi, '#e8f5e9');
    // Replace older blue variants
    scoped = scoped.replace(/#1e5bb8/gi, '#0a4409');
    scoped = scoped.replace(/#2196f3/gi, '#0a4409');
    ...
    codeBlocks.forEach(code => {
        if (!code.classList.contains('formatted')) {
            if (code.tagName === 'PRE') {
                ...
                code.style.borderLeft = '4px solid #0a4409';
            } else {
                code.style.backgroundColor = '#f5f5f5';
...
function displaySectionGroup(groupData) {
    ...
    const pageHeader = document.getElementById('page-header');
    if (pageHeader) {
        pageHeader.innerHTML = `<h1 style="color: white !important; font-size: 42px !important; font-weight: 600 !important;">${groupData.name}</h1>`;
        pageHeader.style.display = 'flex';
    }
    ...
    const contentContainer = document.getElementById('page-content-container');
    if (contentContainer) {
        contentContainer.innerHTML = '<div id="dynamic-sections-container" class="dynamic-sections-container"></div>';
        ...
        if (groupData.sections && Array.isArray(groupData.sections)) {
            groupData.sections.forEach(section => {
                SectionRenderer.render(section, dynamicContainer);
            });
        } else {
            dynamicContainer.innerHTML = '<p>No sections found in this group.</p>';
        }
    }
}
